...
description: Bu rapor, Badstore 1.2.3 platformu üzerinde gerçekleştirilen bir SQL Injection tabanlı sızma testi sürecini detaylandırmaktadır. Test sırasında, sistemdeki kullanıcı doğrulama mekanizmasının güvenlik zafiyetleri, veritabanı yapılandırması ve dosya okuma gibi kritik açıklar incelenmiştir. Elde edilen bilgiler, hassas kullanıcı verilerinin ve sistem dosyalarının izinsiz olarak erişilebilir olduğunu göstermiştir. Bu çalışma, sistem güvenliğini artırmak için gerekli önlemlerin alınmasının önemini vurgulamaktadır.
