...
import { getLanguageIcon } from '@/script/utils/language-icons'
...
const WakatimeGraph = ({ omitLanguages = [] }: Props) => {
    ...
    const CustomYAxisTick = ({ x, y, payload }: any) => {
        ...
        return (
            <g transform={`translate(${x},${y})`}>
                ...
                <foreignObject width={20} height={20} x={-26} y={-8}>
                    {icon ? (
                        React.cloneElement(icon, { size: 16, color: '#E9D3B6' })
                    ) : (
                        <text x={8} y={12} fill='#E9D3B6' fontSize='12' textAnchor='middle' dominantBaseline='central'>
    ...
    return (
        <ChartContainer config={chartConfig} className='size-full p-4'>
            <BarChart accessibilityLayer data={languages} layout='vertical' margin={{ left: -10, right: 10 }}>
                ...
                <Bar dataKey='hours' fill='var(--color-hours)' radius={[8, 8, 8, 8]} isAnimationActive={false}>
                    <LabelList
                        dataKey='hours'
                        position='right'
